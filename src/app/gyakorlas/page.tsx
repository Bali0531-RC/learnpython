    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-24 pt-10 sm:px-10 lg:px-12">
      ...
      <section className="section-card p-8 sm:p-10">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="eyebrow">Első interaktív slice</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-tight">
              Már most megnyitható a teljes szélességű feladatlap, alatta a kódszerkesztővel és a futtatópanellel.
            </h2>
          </div>
          <p className="max-w-xl text-sm leading-6 text-[var(--muted)]">
            A rendszer már kezeli a kezdőbarát stdin/stdout feladatokat és az első fájlos emelt gyakorlófeladatot is. A legtöbb gyakorló tasknál a Futtatás publikus mintákkal megy, a Pontozott beküldés pedig külön rejtett csomagot használ. A sandbox továbbra is tiltja az importot, a külső folyamatindítást és a tetszőleges fájlelérést.
          </p>
        </div>

        <div className="mt-6 grid gap-5 xl:grid-cols-3">
          {practiceTasks.map((task) => (
            <article key={task.id} className="surface-soft flex h-full flex-col rounded-[1.8rem] border border-[var(--line)] p-6">
